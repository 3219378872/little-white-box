...
- 新客户端通过 `clientProtocolVersion=2` 启用结构化问答与原文摘录卡片。回答接口为
  `POST /api/v2/assistant/runs/:id/answers`；新流程在校验后原子发布正文、引用和卡片。
- `just e2e-agent-research` 使用本机严格 fixture，验证社区来源、互联网补充、外部故障、伪造引用、
  问答恢复与历史快照。仅处理带测试标记的模型请求；临时 provider 与 Tavily endpoint 只在子进程
  环境覆盖，结束后恢复原 worker 配置，不改 env 文件或输出凭据。
- `TAVILY_ENDPOINT` 为可选服务端覆盖，空值继续使用原 Tavily 默认服务；不是网页内容抓取地址。
- 旧 Assistant 清库补丁只有在迁移标志已存在或业务库确实为空时才允许自动重放。标志缺失且存在
  数据时拒绝自动清库；本次新增结构为幂等增量迁移。
- Flutter Web 当前采用 hash 路由，助手浏览器入口为 `/#/messages/assistant`。原文详情通过应用内
